...
static IPuss* puss = NULL;
static PussFileBrowser _self;
...
static void fill_subs(PussFileBrowser* self, GFile* dir, GtkTreeIter* parent_iter) {
	...
	for( p=dir_infos; p; p=p->next ) {
		...
		if( subfile ) {
			...
			gtk_tree_store_set( self->store, &subiter, 1, NULL, 2, subfile, -1 );
		}
	}
...
static void fill_root(PussFileBrowser* self) {
	...
	for(p=mounts; p; p=p->next ) {
		...
		if( file ) {
			...
			gtk_tree_store_set( self->store, &subiter, 1, NULL, 2, file, -1 );
		}
	}
...
static void build_ui(PussFileBrowser* self, const char* module_path) {
	gint w, h;
	GtkBuilder* builder;
	gchar* ui_pth;

	builder = gtk_builder_new();
	ui_pth = g_build_filename(module_path, "filebrowser.ui", NULL);
...
G_MODULE_EXPORT void filebrowser_cb_refresh_button_clicked(GtkWidget* w, gpointer user_data) {
	do_refresh((PussFileBrowser*)user_data);
}

G_MODULE_EXPORT void filebrowser_cb_row_activated(GtkTreeView* tree_view, GtkTreePath* path, GtkTreeViewColumn* column, gpointer user_data) {
...
G_MODULE_EXPORT void filebrowser_cb_row_collapsed(GtkTreeView* tree_view, GtkTreeIter* iter, GtkTreePath* path, gpointer user_data) {
	// TODO free sub tree & add [loading] node
}

G_MODULE_EXPORT void filebrowser_cb_row_expanded(GtkTreeView* tree_view, GtkTreeIter* iter, GtkTreePath* path, gpointer user_data) {
	PussFileBrowser* self = (PussFileBrowser*)user_data;
	ensure_fill_subs(self, iter);
}

G_MODULE_EXPORT gboolean filebrowser_view_cb_keypress(GtkWidget* w, GdkEventKey* event, gpointer user_data) {
	PussFileBrowser* self = (PussFileBrowser*)user_data;
...
G_MODULE_EXPORT void puss_module_uninit(void) {
	PussFileBrowser* self = &_self;
